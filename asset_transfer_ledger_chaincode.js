...
class Chaincode extends Contract {
	...
	async CreateAsset(ctx, hashId, fileName, fileType, fileExtenstion, fileSize, uploadTime, ipfsPath, ownerInfo, description) {
		const exists = await this.AssetExists(ctx, hashId);
		if (exists) {
			throw new Error(`The asset ${hashId} already exists`);
		}
		...
		// === Save asset to state ===
		await ctx.stub.putState(hashId, Buffer.from(JSON.stringify(asset)));
		let indexName = 'ownerInfo~hashId';
		let colorNameIndexKey = await ctx.stub.createCompositeKey(indexName, [asset.ownerInfo, asset.hashId]);
	...
	async DeleteAsset(ctx, id) {
		...
		// delete the index
		let indexName = 'ownerInfo~hashId';
		let colorNameIndexKey = ctx.stub.createCompositeKey(indexName, [assetJSON.ownerInfo, assetJSON.hashId]);
	...
	async TransferAsset(ctx, assetName, newOwner, newDesc) {
		...
		assetToTransfer.ownerInfo = newOwner; //change the owner
		assetToTransfer.description = newDesc;
	...
	async AssetExists(ctx, hashId) {
		// ==== Check if asset already exists ====
		let assetState = await ctx.stub.getState(hashId);
		return assetState && assetState.length > 0;
	}

	// AssetExists returns true when asset with given ID exists in world state
	async queryByCurrentState(ctx, hashId) {
		// ==== Check if asset already exists ====
		let res = await ctx.stub.queryByCurrentState(1);
		console.log(hashId);
	}
	...
	async InitLedger(ctx) {
		const mediaAssets = [
            {
                hashId : 'QmVozEUQhkPzYZS5eDKGAq14wdqFiM4ocJHRVUhbFgsMkW',
                fileName : 'ipfstest.txt',
                fileType : 'text/plain',
                fileExtenstion : '.txt',
                fileSize : '2887',
                uploadTime : 'Sat Jul 24 2021 01:01:48 GMT+0100 (British Summer Time)',
                ipfsPath: 'http://127.0.0.1:8080/ipfs/QmVozEUQhkPzYZS5eDKGAq14wdqFiM4ocJHRVUhbFgsMkW',
                ownerInfo:'Owner name',
                description: 'complete file description.'
            }  
        ];
